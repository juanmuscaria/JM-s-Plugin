...
public class Tpa implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender commandSender, Command command, String label, String[] args) {
        ...
        if (p.equals(p2)){
            p.sendMessage(ChatColor.RED + "Você não pode enviar um pedido de teleporte para você mesmo.");
            return true;
        }
        TpaData a = new TpaData(p.getName(), p2.getName());
        if (JMEssentials.tpaCache.containsKey(p2.getName())){
            p.sendMessage(ChatColor.RED + "Esse jogador já tem um pedido de teleporte pendente.");
            return true;
        }
        JMEssentials.tpaCache.put(p2.getName(), a);
        p.sendMessage(ChatColor.GREEN + "Pedido enviado com sucesso.");
        if (!p2.hasPermission(command.getPermission())) return true;
        new JSONChat().addText(new Text("Você recebeu um pedido de teleporte do jogador:" + p.getName()).setBold(true).setColor(Colors.Yellow)).sendTo(p2);
        ...
        new JSONChat().addText(new Text("(?)", Colors.Gray).setHoverEvent(new Event("show_text", "Dica: Você pode clicar para aceitar/recusar o pedido").toJson())).sendTo(p2);
        return true;
    }
}
